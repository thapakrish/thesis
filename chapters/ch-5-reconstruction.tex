...
As described in section \ref{ssec:trigger}, slink is designed to save only some of the hit information based on some predefined trigger setup. Fig. \ref{fig:pBXslink} shows the number of tracks  per bunch crossing. The tall towers represent the the colliding bunches and the rest are the non-filled bunches. Contribution from non-filled BX is very low.
%Slink data is saved in binary format that can be used in "offline" analysis. 
...
Given a set of hits on a set of planes, tracking algorithm is used as described in \ref{sec:telAlign} to make tracks which can then be used to parametrize the measurement dataset. For each track, reconstruction of slopes and residuals in x and y direction, and the beamspot position are described below.
...
%%
%So, this track now has 3 variables, (x at z=0,y at z=0,slope-x,slope-y,residual-x,residual-y) with these variables, we can find out where the hit happened in each plane. In fact, since we know the global position of each telescope, slope and residuals themselves are enough to charactering everything about the track, given that we know which telescope this track came from.
%%
%So, the most independent variables that we are interested in are slopes and residuals.
%%
%%
%%
%We take the three hits on three different planes (select these only), and extrapol


\subsubsection{Track Beamspot}
Beamspot is defined as the position of track in the x-y plane at the interaction point i.e. at z=0 in CMS coordinate system. Figure \ref{fig:44beamspots} shows the beamspot(cm) seen by the telescopes in both sides of the interaction point from Fill 4444, 2015.  Inner blue ring is an ellipse drawn such that major and minor axis represent $3 \sigma$ of x and y mean separately. The center of the ellipse is positioned at $(\bar{x},\bar{y})$. Most of the tracks seem to be coming from within 2.25 cm of the IP, the beampipe size.  The "arms" arise due to the acceptance region of individual telescopes, which is not quite symmetric due to the absence of two telescopes in one side. 
